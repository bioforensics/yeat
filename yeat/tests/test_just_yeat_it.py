...
def test_paired_end_assemblers(algorithm, capsys, tmp_path):
    wd = str(tmp_path)
    arglist = ["-o", wd, data_file("short_reads_1.fastq.gz"), data_file("short_reads_2.fastq.gz")]
    if algorithm != "spades":
...
@pytest.mark.short
@pytest.mark.parametrize("algorithm", ["spades", "megahit", "unicycler"])
def test_get_algorithm(algorithm, tmp_path):
    wd = str(tmp_path)
    arglist = ["-o", wd, data_file("short_reads_1.fastq.gz"), data_file("short_reads_2.fastq.gz")]
    if algorithm != "spades":
        arglist.append(f"--{algorithm}")
    args = get_parser().parse_args(arglist)
    assert algorithm == get_algorithm(args)

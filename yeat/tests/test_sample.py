...
def test_check_read_path(read_path):
    label = "sample1"
    data = {"illumina": read_path}
    Sample._check_read_path(label, data)


def test_check_read_path_unable_to_find():
    label = "sample1"
    data = {"illumina": []}
...
def test_check_read_path_found_too_many(tmp_path):
    wd = tmp_path

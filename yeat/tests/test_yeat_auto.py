...
@pytest.mark.parametrize(
    "sequences,expected",
    [
        (["short_reads"], data_file("configs/auto_one_sample.toml")),
        (["short_reads", "Animal_289"], data_file("configs/auto_two_samples.toml")),
        ([data_file("one_sample.txt")], data_file("configs/auto_one_sample.toml")),
        ([data_file("two_samples.txt")], data_file("configs/auto_two_samples.toml")),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (["short_reads"], SHORT_READS, data_file("configs/auto_one_sample.toml")),
        (["short_reads", "Animal_289"], ALL_READS, data_file("configs/auto_two_samples.toml")),
        ([data_file("one_sample.txt")], SHORT_READS, data_file("configs/auto_one_sample.toml")),
        ([data_file("two_samples.txt")], ALL_READS, data_file("configs/auto_two_samples.toml")),
    ],
)

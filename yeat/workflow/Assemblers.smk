...
rule spades:
    ...
    params:
        ...
        # extra_args=lambda wildcards: config["asm"].assemblies[wildcards.label].extra_args
        extra_args=""
    log:
        "analysis/{sample}/yeat/spades/{label}/spades-stdout-err.log"
    shell:
        """
        spades.py -1 {input.reads[0]} -2 {input.reads[1]} -t {threads} -o {params.outdir} {params.extra_args} > {log} 2>&1
        """
